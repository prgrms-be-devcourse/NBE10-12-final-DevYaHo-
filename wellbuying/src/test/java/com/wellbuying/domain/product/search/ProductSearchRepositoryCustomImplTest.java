...
class ProductSearchRepositoryCustomImplTest extends AbstractIntegrationTest {
    ...
    void search_키워드가_productName에_포함된_APPROVED_상품을_반환한다() {
        ...
        CursorPageResponse<ProductSearchResponse> result =
                productSearchRepository.search("유기농 비타민C", SearchSortType.RELEVANCE, null, 20, ProductSearchFilter.none());
    ...
    void search_키워드가_description에만_있어도_반환된다() {
        CursorPageResponse<ProductSearchResponse> result =
                productSearchRepository.search("비타민", SearchSortType.RELEVANCE, null, 20, ProductSearchFilter.none());
    ...
    void search_PENDING_상품은_결과에서_제외된다() {
        CursorPageResponse<ProductSearchResponse> result =
                productSearchRepository.search("콜라겐", SearchSortType.RELEVANCE, null, 20, ProductSearchFilter.none());
    ...
    void search_REJECTED_상품은_결과에서_제외된다() {
        CursorPageResponse<ProductSearchResponse> result =
                productSearchRepository.search("마그네슘", SearchSortType.RELEVANCE, null, 20, ProductSearchFilter.none());
    ...
    void search_커서로_다음_페이지를_조회하면_결과가_겹치지_않는다() {
        ...
        CursorPageResponse<ProductSearchResponse> first =
                productSearchRepository.search("비타민", SearchSortType.RELEVANCE, null, 1, ProductSearchFilter.none());
        ...
        CursorPageResponse<ProductSearchResponse> second =
                productSearchRepository.search("비타민", SearchSortType.RELEVANCE, first.nextCursor(), 1, ProductSearchFilter.none());
    ...
    void search_activeGroupBuyOnly가_true면_hasActiveGroupBuy가_true인_상품만_반환한다() {
        ...
        try {
            CursorPageResponse<ProductSearchResponse> result =
                    productSearchRepository.search("액티브공동구매상품", SearchSortType.RELEVANCE, null, 20,
                            new ProductSearchFilter(null, null, null, true));
    ...
    void search_POPULAR_정렬시_조회수_내림차순으로_반환된다() {
        ...
        try {
            CursorPageResponse<ProductSearchResponse> result =
                    productSearchRepository.search("파퓰러정렬테스트", SearchSortType.POPULAR, null, 20, ProductSearchFilter.none());
    ...
    void search_categoryId_필터로_해당_카테고리_상품만_반환한다() {
        ...
        productSearchRepository.saveAll(List.of(
                docWithCategory(idA, "카테고리테스트A", "카테고리 필터 검증", "APPROVED", 1L),
                docWithCategory(idB, "카테고리테스트B", "카테고리 필터 검증", "APPROVED", 2L)));
        operations.indexOps(ProductSearchDocument.class).refresh();
        try {
            CursorPageResponse<ProductSearchResponse> result =
                    productSearchRepository.search("카테고리테스트", SearchSortType.RELEVANCE, null, 20,
                            new ProductSearchFilter(2L, null, null, false));
            assertThat(result.content()).hasSize(1);
    ...
    void search_가격대_필터로_해당_범위_상품만_반환한다() {
        ...
        try {
            CursorPageResponse<ProductSearchResponse> result =
                    productSearchRepository.search("가격대테스트", SearchSortType.RELEVANCE, null, 20,
                            new ProductSearchFilter(null, 10000, 20000, false));
            assertThat(result.content()).hasSize(1);
    ...
    private ProductSearchDocument doc(long id, String name, String description, String status, boolean hasActiveGroupBuy) {
        return new ProductSearchDocument(id, name, description, 1L, status, 10000, 0L, "url", 1L, LocalDateTime.now(),
                ...
                hasActiveGroupBuy ? LocalDateTime.of(2026, 9, 20, 0, 0) : null,
