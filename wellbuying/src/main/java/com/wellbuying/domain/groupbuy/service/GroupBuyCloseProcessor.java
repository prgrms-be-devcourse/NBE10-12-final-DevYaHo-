...
public class GroupBuyCloseProcessor {
    ...
    // 최소 수량 달성 - SUCCESS 확정 + 확정 참여자 전원에게 최종 단가를 UPDATE 한 문장으로 반영 (finalPrice와
    // confirmedParts는 호출 측이 배치 조회로 미리 구해둔 값을 그대로 넘겨받으므로 여기서 추가 조회가 없다)
    @Transactional
    public GroupBuy closeSucceeded(Long groupBuyId, int finalPrice, List<GroupBuyPart> confirmedParts) {
