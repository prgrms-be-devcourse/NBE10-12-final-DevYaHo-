...
public interface PaymentEventOutboxRepository extends JpaRepository<PaymentEventOutbox, Long> {
    ...
    // 아직 Kafka로 발행하지 못했고 재시도 한도(PaymentEventOutbox.MAX_RETRY_COUNT)를 넘지 않은 이벤트를
    // 오래된 순으로 조회 - PaymentOutboxRelay가 한 번의 실행에서 처리할 최대 건수를 limit으로 제한한다.
    // 재시도 한도를 넘긴 poison pill은 조회 대상에서 빠진다
    List<PaymentEventOutbox> findByPublishedAtIsNullAndRetryCountLessThanOrderByIdAsc(int retryCount, Limit limit);

    // PaymentOutboxRelay가 배치를 병렬 발행한 뒤 성공한 건들의 published_at을 한 번의 UPDATE로 채운다 -
    // 건마다 개별 save()를 호출하면 그만큼 개별 UPDATE 왕복이 발생하므로 벌크로 묶는다
    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("UPDATE PaymentEventOutbox e SET e.publishedAt = :publishedAt WHERE e.id IN :ids")
    ...
    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("UPDATE PaymentEventOutbox e SET e.retryCount = e.retryCount + 1 WHERE e.id IN :ids")
