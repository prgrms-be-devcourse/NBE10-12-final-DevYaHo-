...
// PaymentOutboxRelay가 배치 전체를 병렬로 payment-events에 발행한 결과(성공/실패)를 DB에 반영한다.
// 건별로 개별 UPDATE를 내보내지 않도록, 성공/실패 각각을 한 번의 벌크 UPDATE로 묶어서 처리한다.
// (GroupBuyOutboxDispatcher와 같은 방식)
@Component
public class PaymentOutboxDispatcher {
    ...
    public PaymentOutboxDispatcher(PaymentEventOutboxRepository outboxRepository) {
        this.outboxRepository = outboxRepository;
    }

    // 발행 성공 - published_at을 한 번의 UPDATE로 채워, 다음 릴레이 주기의 미발행 조회 대상에서 한꺼번에 빠지게 한다
    @Transactional
    public void markPublished(List<PaymentEventOutbox> events) {
        if (events.isEmpty()) {
            return;
        }
        outboxRepository.markPublished(events.stream().map(PaymentEventOutbox::getId).toList(), LocalDateTime.now());
    }
    ...
    public void recordFailures(List<DispatchFailure> failures) {
        ...
        for (DispatchFailure failure : failures) {
            PaymentEventOutbox event = failure.event();
            event.recordFailure();
            if (event.isRetryExhausted()) {
                log.error("결제 아웃박스 이벤트 발행 포기(최대 {}회 재시도 초과) - id={}, groupBuyId={}, eventType={}",
                        PaymentEventOutbox.MAX_RETRY_COUNT, event.getId(), event.getGroupBuyId(),
                        event.getEventType(), failure.cause());
            } else {
                log.error("결제 아웃박스 이벤트 발행 실패({}번째 재시도) - id={}, groupBuyId={}, eventType={}",
    ...
    public record DispatchFailure(PaymentEventOutbox event, Throwable cause) {
    }
}
