...
public class GroupBuySeedRunner implements ApplicationRunner {
    ...
    private List<SeedProduct> createSeedProducts(Long producerId) {
        List<SeedItem> items = List.of(
                ...
                // 페이지네이션 확인용으로 추가한 항목들 - seedParticipation()의 plan이 원래 8개(인덱스 0~7)의
                // 위치를 참조하므로 반드시 뒤에 이어붙인다(중간에 끼워 넣지 않는다)
                new SeedItem("제주 유기농 감귤", "식품", "새콤달콤 제주 유기농 감귤 3kg", 5, 40,
                        -8, 10, tiers(5, 19000, 15, 16500, 40, 14500)),
                new SeedItem("리넨 반팔 셔츠", "패션", "구김 적은 여름 리넨 반팔 셔츠", 5, 25,
                        -12, 12, tiers(5, 32000, 10, 28000, 25, 25000)),
                new SeedItem("핸드메이드 도자기 머그", "생활", "매일 손이 가는 핸드메이드 도자기 머그", 5, 35,
                        -18, 7, tiers(5, 17000, 15, 14500, 35, 12900)),
                new SeedItem("저온숙성 원목 도마", "기타", "위생적인 저온숙성 원목 도마", 5, 20,
                        -4, 21, tiers(5, 26000, 10, 23000, 20, 20000)),
                new SeedItem("무항생제 유정란", "식품", "산란 당일 발송하는 무항생제 유정란 30구", 5, 45,
                        -22, 5, tiers(5, 14000, 20, 12000, 45, 10500)),
                new SeedItem("삼베 매트리스 커버", "생활", "통풍 잘 되는 여름 삼베 매트리스 커버", 5, 20,
                        -28, 13, tiers(5, 35000, 10, 31000, 20, 27000)),
                new SeedItem("울 혼방 니트 가디건", "패션", "가볍게 걸치는 울 혼방 니트 가디건", 5, 15,
                        -2, 17, tiers(5, 42000, 8, 38000, 15, 34000)),
                new SeedItem("천연 왁스 캔들 세트", "기타", "은은한 조도의 천연 왁스 캔들 세트", 5, 25,
                        -35, 4, tiers(5, 19500, 15, 17000, 25, 15000))
        );
